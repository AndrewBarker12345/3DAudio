/*
 ...
 Provides real-time read and blocking or non-blocking
 write access to shared data in concurrent environments.
...
// a recursive mutex that can identify it's owner thread
...
// a concurrent resource that can always immediately produce a locked copy of the resource for realtime thread-safe use, so long as numThreads >= the actual number of threads that are simultaneously accessing the resource
template <typename T, const std::size_t numThreads>
class RealtimeConcurrent
{
public:
    RealtimeConcurrent() noexcept {}
    RealtimeConcurrent(const T& resource)
    {
        for (auto& c : copies)
            c.load(resource);
    }
...
//    /** this will be locked for the duration of the current scope */
////    template <class T>
//    class ScopeLockedResource {
//    friend class RealtimeConcurrent;
//        ScopeLockedResource(T* res, Mutex&& lock) : resource(res), lock(std::move(lock)) {}
//        //ScopeLockedResource(ScopeLockedResource&& moveFrom) : resource(moveFrom.resource), locker(std::move(moveFrom.locker)) {}
//        T* resource;
//        //std::lock_guard<Mutex> locker;
//        Mutex lock;
//    public:
//        operator T&() { return resource ? *resource : T(); }
//        operator const T&() const { return resource ? *resource : T(); }
//        operator T*() noexcept { return resource; }
...
//        // alternatively, a straightforward blocking update
//        const std::lock_guard<std::mutex> lockedForUpdate (updateLock);
//        T* copyToUpdate = nullptr;
//        for (auto& c : copies) {
//            if (c.isSame(updatedCopy)) {
...
/* example code:
 // the data that needs to be shared between numThreads threads
 ConcurrentResource<Thing, numThreads> c_resource;
 
 // this function has a fixed upper bound execution time, the critical requirement for real time systems
 void realtimeFunction()
 {
    Thing* copy = nullptr;
    std::lock_guard<Mutex> lock (c_resource.get(copy)); // won't fail to lock the copy that is returned so long as numThreads >= the actual number of threads trying to access the resource
    if (copy == nullptr) // but maybe it does fail...
        return;
 
    // use the copy (read and/or write) ...
    bool copyWrittenTo = ... ;
 
    if (copyWrittenTo) { // try to update other copies with changes
        bool failedToUpdateAllCopies = c_resource.tryToUpdate(copy);
        if (failedToUpdateAllCopies)
            // handle somehow if you want...
    }
 }
 
 // this function has no guaranteed execution time!, BUT it ensures that writes to the resource copy are propagated to all other copies
 void nonRealtimeFunction()
 {
    Thing* copy = nullptr;
    std::lock_guard<Mutex> lock (c_resource.get(copy)); // won't fail
    if (copy == nullptr)
        return;
 
    // write to the copy ...
 
    // this will block until all copies are updated
...
////  ConcurrentResource.h
////
////  Created by Andrew Barker on 1/20/16.
////
////
///*
